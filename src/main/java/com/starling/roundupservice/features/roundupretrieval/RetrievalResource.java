...
public class RetrievalResource
{
    ...
    public ResponseEntity<RoundupRetrievalResponse> retrieveRoundupGoal(@PathVariable("accountUid") final String accountUid,
                                                                        @RequestHeader("Authorization") String bearerToken)
    {
        try
        {
            var roundupInfo = retrievalService.retrieveRoundup(accountUid, bearerToken);
        ...
        catch (ClientException e)
        {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(RoundupRetrievalResponse.builder().errors(e.getError()).build());
        }
        catch (
                ServerException e)
        {
            log.error("There was an error on our side processing your request for roundup retrieval for account {}", accountUid, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(RoundupRetrievalResponse.builder().errors(e.getError()).build());
        }
        catch (Exception e)
        {
